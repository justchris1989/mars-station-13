...
/obj/machinery/computer/prison_shuttle/verb/take_off()
	...
	if(zlevel == 5)
		A = locate(/area/shuttle/prison/mars)
	else
	...
	// And 5 is the Mars Z level
	// Now uses keelin's move_contents_to

	if(zlevel == 5)
		start_location = locate(/area/shuttle/prison/mars)
		...
		start_location = locate(/area/shuttle/prison/transfer)
		end_location = locate(/area/shuttle/prison/mars)
		start_location.move_contents_to(end_location)
	...
	for(var/mob/M in A)
		M.show_message("\red Mars shuttle has arrived at destination!")
...
/obj/machinery/computer/prison_shuttle/verb/restabalize()
	...
	for(var/mob/M in A)
		M.show_message("\red <B>Restabilizing Mars shuttle atmosphere!</B>")
*/
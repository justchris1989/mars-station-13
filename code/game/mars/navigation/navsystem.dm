/obj/item/device/mars/navigation/pointer
	name = "Mars Navigation System"
	icon = 'device.dmi'
	icon_state = "mpinoff"
	...
	throw_range = 20
	m_amt = 500
	var/obj/machinery/mars/mbeacon/the_disk = null
	var/active = 0
...
//			active = 0
			...
			usr << "\red Beacon signal lost. Plese only use the system on the Mars."
			return
		if(!active) return
		if(!the_disk)
			the_disk = locate()

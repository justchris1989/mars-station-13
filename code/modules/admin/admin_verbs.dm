...
//Removed get mob and stuff, it causes trouble, i.e. Twatt
...
/client/proc/launchvolcano()
	set category = "Special Verbs"
	set name = "Launch Volcano"
	set desc = "Ruin everybodys game"
	if(!src.authenticated || !src.holder)
		src << "Only administrators may use this command."
		return
	volcano()

/client/proc/checkowl(var/mob/M in world)
	set category = "Admin"
	set name = "Check OWL"
	set desc = "Check O.W.L.S. status of a user"
	if(!src.authenticated || !src.holder)
		src << "Only administrators may use this command."
		return
	var/output = "[M] is"
	if(verify_officer(M)) output += "a officer,"
	if(!verify_officer(M)) output += "not a officer,"
	if(verify_commander(M)) output += "a commander."
	if(!verify_commander(M)) output += "not a commander."
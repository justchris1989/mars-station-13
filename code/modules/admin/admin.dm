...
// Now isn't that much better? IT IS NOW A PROC, i.e. kinda like a big panel like unstable
	...
	if (href_list["adminplayeropts"])
		...
		if (href_list["prayeropts"])
			var/mob/D = locate(href_list["prayeropts"])
			var/d4t = "<html><head><title>Prayer options for [D.key]</title></head>"
			var/f0o = "\[ "
			if (ismob(D) && D.client)
				if(!D.client.authenticated && !D.client.authenticating)
					f0o += text("<A HREF='?src=\ref[src];adminauth=\ref[D]'>Authorize</A> | ")
				else
					f0o += text("<B>Authorized</B> | ")
				f0o += text("<A HREF='?src=\ref[src];prom_demot=\ref[D.client]'>Promote/Demote</A> | ")
				if(!istype(D, /mob/new_player))
					if(!istype(D, /mob/living/carbon/monkey))
						f0o += text("<A HREF='?src=\ref[src];monkeyone=\ref[D]'>Monkeyize</A> | ")
					else
						f0o += text("<B>Monkeyized</B> | ")
					if(istype(D, /mob/living/silicon/ai))
						f0o += text("<B>Is an AI</B> | ")
					else if(istype(D, /mob/living/carbon/human))
						f0o += text("<A HREF='?src=\ref[src];makeai=\ref[D]'>Make AI</A> | ")
					f0o += text("<A HREF='?src=\ref[src];tdome1=\ref[D]'>Thunderdome 1</A> | ")
					f0o += text("<A HREF='?src=\ref[src];tdome2=\ref[D]'>Thunderdome 2</A> | ")
					f0o += text("<A HREF='?src=\ref[src];sendtoprison=\ref[D]'>Prison</A> | ")
					f0o += text("<A HREF='?src=\ref[src];sendtomaze=\ref[D]'>Maze</A> | ")

					f0o += text("<A HREF='?src=\ref[src];revive=\ref[D]'>Heal/Revive</A> | ")
				else
					f0o += text("<B>Hasn't Entered Game</B> | ")
				f0o += text("<A HREF='?src=\ref[src];forcespeech=\ref[D]'>Say</A> | ")
				f0o += text("<A href='?src=\ref[src];mute2=\ref[D]'>Mute: [(D.muted ? "Muted" : "Voiced")]</A> | ")
				f0o += text("<A href='?src=\ref[src];jobban2=\ref[D]'>Jobban</A> | ")
				f0o += text("<A href='?src=\ref[src];boot2=\ref[D]'>Boot</A> | ")
			f0o += text("<A href='?src=\ref[src];jumpto=\ref[D]'>Jump to</A> | ")
			f0o += text("<A href='?src=\ref[src];newban=\ref[D]'>Ban</A> \]")
			d4t += text("<body>[foo]</body></html>")
			usr << browse(dat, "window=prayeropts;size=480x100")
			...
			switch(href_list["secretsfun"])
				...
				*/
				...
				if("activateprison")
					...
					var/area/start_location = locate(/area/shuttle/prison/mars)
					...
					for(var/atom/movable/AM as mob|obj in A)
						...
						AM.Move()
					*/
					message_admins("\blue [key_name_admin(usr)] sent the Mars shuttle to the station.", 1)
				if("deactivateprison")
					var/area/start_location = locate(/area/shuttle/prison/station)
					var/area/end_location = locate(/area/shuttle/prison/mars)
					start_location.move_contents_to(end_location)
					/*
					var/A = locate(/area/shuttle_prison)
					for(var/atom/movable/AM as mob|obj in A)
						...
						AM.Move()
					*/
					message_admins("\blue [key_name_admin(usr)] sent the Mars shuttle to Mars.", 1)
...
<A href='?src=\ref[src];secretsfun=toggleprisonstatus'>Toggle Mars Shuttle Status(Use with S/R)</A><BR>
<A href='?src=\ref[src];secretsfun=activateprison'>Send Mars Shuttle</A><BR>
<A href='?src=\ref[src];secretsfun=deactivateprison'>Return Mars Shuttle</A><BR>
